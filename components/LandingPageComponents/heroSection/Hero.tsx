...
function Hero(props: Props) {
    ...
    <div className="mx-auto max-w-7xl flex flex-col justify-center items-center">
        <div className="container w-full pt-32 pb-8 mt-px text-left sm:px-4 md:text-center">
            ...
            <div className="mx-auto mt-5 text-gray-400 md:mt-8 md:max-w-lg md:text-center md:text-xl">Simplifying the creation of landing pages, blog pages, application pages and so much more with (<span className='font-bold text-slate-600'>Tailwind CSS and ReactJs</span>) based components!</div>
        </div>
        <Link href='/componentsPage'><button className='font-semibold w-fit text-base rounded-full cursor-pointer border-2 hover:scale-[102%] px-8 py-3 hover:bg-white hover:border-slate-800 hover:text-slate-800 bg-slate-800 text-white active:scale-95 transition-all duration-300 ease-in-out'>Visit Components</button></Link>
    </div>
</section>
    )
}
